...
DATA_FILE = "./data/food-mother-classified.csv"

# ----------------------------------------------------------------
# Nutrient columns from your dataset
# ----------------------------------------------------------------
NUTRIENT_COLS = [
...
def generate_recommendations(deficiency_text, profile, top_n=5):
    ...
    # Rename Gemini columns to simplified form
    ...
    # Filter by mother's profile
    def matches(profile, row):
        state_ok = profile["state"] in str(row["states"])
    ...
    df_filtered["final_score"] = (
        0.5 * df_filtered["nutrient_score"] + 0.15 + 0.1 + 0.1 + 0.15
    )
